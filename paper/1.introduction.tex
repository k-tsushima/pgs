...
The synchronization of data is a common problem. In the database community this problem is known as “the view update problem” and has been investigated for a long time~\cite{Bancilhon:1981:USR:319628.319634}. Bidirectional transformation (BX) is an approach to solve this problem.
...
If the programs are simple like these running examples, we can transform left associative compositions to right associative. However, this is not always possible. \textcolor{red}{For example, let us consider the following 'breverse' program.}

%$\tab
%bsnoc = Rearr\\
%bfoldr bx = Case \ f_1 \ f_2 \ bx_s \text{where: } f_1 = , f_2% = \\

%breverse = RearrS \ f_1 \ f_2 \ bx_s \ \text{where: } f_1 = \l%ambda s.(s.()), \, f_2 = \lambda (s.()).s,\\
%\qtab bx_s = bfoldr bsnoc$

\input{breverse.tex}

\noindent \textcolor{red}{In this program, recursion occurs on the left-hand side of composition. Because of priority, it is impossible to tranform this program to right associative compositions. At the same time, we can not apply fusion \cite{} to this program because this program is dynamically produced depending on the length of the input list.}
Therefore it is important to have a fast evaluation method for left associative compositions.
