...
\begin{table}[]
    ...
    \resizebox{\textwidth}{!}{%
        \begin{tabular}{|l|l|l|l|}
            ...
            breverse & l, R & xpg $<$ kpg $<$ minbigul $<$ cpg $<$ pg & xpg (ND) $<$ kpg (ND) $<$ cpg (ND) $<$ minbigul (ND) $<$ pg (ND) \\ \hline
            \multicolumn{4}{r}{} \\
...
\begin{itemize}
    \item Number of calls
        \begin{itemize}
            \item No(cpg) = No(kpg) = No(xpg) = No(put) (\textcolor{blue}{come from function definitions})
            \item For right-associative \& non-recursive composition (rassoc-comp-replace, rassoc-comp-phead): No(get), No(put), No(pg) are linear where No(get) $<$ No(put) $<$ No(pg)
            \item For left-associative \& non-recursive composition (lassoc-comp-replace, lassoc-comp-phead): No(put) is still linear, No(get) is quadratic, No(pg) is under exponential distribution. No(put) $<$ No(get) $<$ No(pg)
            \item For left-associative \& recursive composition (breverse): we still have No(put) $<$ No(get) $<$ No(pg) and all three are non-linear. In case of breverse, No(put) is quadratic, No(get) is cubic and No(pg) is exponential.
        \end{itemize}
    \item Evalutation times (s)
        \begin{itemize}
            \item For left-associative compositions:\\
            - pg time is exponential (\textcolor{blue}{since the number of pg calls in these cases are exponential}) \\
            \item For left-associative \& non-recursive compositions:\\
            - kpg time are are approximate to xpg time, and they are the most efficient.\\
            - In case of lassoc-comp-replace, kpg time and xpg time are linear, cpg time and minbigul time are non-linear. (\textcolor{blue}{because the input size is constant, No(get) is quadratic for minbigul while the number of redundant evaluations in cpg is \underline{linear} and ... ?})\\
            - In case of lassoc-comp-phead, because of the data size is linear and the numbers of function calls are either linear or quadractic or exponential, then all times are non-linear.

            \item For right-associative \& non-recursive compositions:\\
            - minbigul time, pg time and xpg time are linear \\
            - In case of rassoc-comp-replace, all times are small enough to consider all them as linear \\
            - In case of rassoc-comp-phead, cpg time and kpg time are quadratic (\textcolor{blue}{because data size is linear, the number of redundant evaluations in cpg and kpg are linear})

            \item For left-associative \& recursive composition (breverse):\\
            - all times should be non-linear (\textcolor{blue}{because there is no linear number of function calls})
        \end{itemize}
    \item Memory allocation (MBytes)
        \begin{itemize}
            \item Memory usage depends on the input size and the number of composition
            \item In general, minbigul uses less memory than the others for non-recursive compositions (all tests except breverse)
            \item pg only uses memory efficient when handling right-associative composition 
            \item cpg \& kpg are worst in memory allocation for right-associative
            \item For left-associative \& recursive composition (breverse), minbigul uses significantly more memory than kpg as well as xpg.
        \end{itemize}
\end{itemize}
