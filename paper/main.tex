...
%{Zhenjiang Hu\inst{1}\orcidID{0000-0002-9034-205X}}
%{Robert Gl\"{u}ck\inst{2}\orcideID{0000-0001-6990-3935}}
%{Kanae Tsushima\inst{3}\orcidID{}}
%{Bach-Trong Nguyen\inst{3}\orcidID{0000-0002-6342-6020}}
...
%15--250 words.
  Bidirectional transformation (BX) is a solution of view update problems and widely used for synchronizing data. Its semantics and correctness are researched well, but the evaluation efficiency is not considered well. As a first attempt we consider efficiency of evaluation in this work. To improve evaluation efficiency, we focus on the evaluation of BX compositions, because its evaluation often causes redundant evaluations. 
  For evaluating BX compositions efficiently, we introduce two memoization strategies. The first strategy, minBigul\_m, uses so simple memoization, but it works well for many BX programs. This keeps the evaluated results in a table and reuse them.  A disadvantage of this is cost for saving to a table and searching. When the inputs are large, the evaluation time becomes slow. To treat large inputs, we introduce the second approach, xpg, that uses tupling and some optimization techniques. In this approach we shrink the size of the evaluated results and keep them in a closure. The experiments of these strategies show the following results: the original strategy is faster for right assoc compositions, minBiGUL\_m is faster for not-large inputs and xpg is faster for large inputs.
