...
Next let us look at another more complex example using a case combinator:

    $\tab bsnoc = Case \ cond_{sv} \ cond_s \ bx_1 \ bx_2$ where:\\
    $\tabs{3} \ cond_{sv} = \lambda s.\lambda v.(\textnormal{length } v = 1), \ cond_s = \lambda (s_1,s_2).(s_2 = [\ ])$\\
    $\tabs{3} \ bx_1 =  Replace$\\
    $\tabs{3} \ bx_2 = RearrS \ f_1 \ f_2 \ bx_s$ where:\\
        $\tabs{4} f_1 = f_2^{-1} = \lambda (x:y:ys).(y,(x:ys))$, \\
        $\tabs{4} bx_s = RearrV \ g_1 \ g_2 \ bx_v$ where:\\
            $\tabs{5} g_1 = g_2^{-1} = \lambda (v:vs).(v,vs)$\\
            $\tabs{5} bx_v = Replace \times bsnoc$
