...
\textcolor{red}{Note that this result focuses on the BX programs that include many compositions. If the number of compositions is small in the BX programs, the original $put$ without any memoization will be fastest, because of the overhead for memoization ($put_m$) and the overhead for keeping complements in closures ($xpg$).}
