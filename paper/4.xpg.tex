...
The first equality is simply based on the definitions of $pg$, $\putbxinline{Skip \ h}$ and $\getbxinline{Skip \ h}$. The second one tuples two results of $put$ and $get$ in the body of the if-expression. This is a trick since in some cases, the result of $pg$ may be undefined although the result is not undefined when evaluating $\getbxinline{Skip \ h}$. The last equality is relatively obvious.
...
A restriction for $\pginline{Case}$ needs to be introduced here. We know that there is one entering condition and one exit condition when evaluating $\putbxinline{Case}$ as well as $\getbxinline{Case}$. If a tupling occurs, there will be 4 combinations from these conditions. This means two entering conditions of $\putbxinline{Case}$ and $\getbxinline{Case}$ are not always simultaneously satisfied. The evaluated branches are distinct in the $put$ and $get$ directions for combinations $((cond_{sv} \ s \ v) \ \&\& \ (not (cond_{s} \ s)))$ and $((not (cond_{sv} \ s \ v)) \ \&\& \ (cond_{s} \ s))$, which are restricted in this paper. This does not happen for the others which is used in the construction of $\pginline{Case}$.
    ...
    \tabs{3} \overset{4}{=} (s_1, v_1) \Leftarrow \pg{bx_1}{s}{construct\_dummy \ v}; \\
...
Since there is no real view, this $dummy$ is necessary to pair with original source $s$ to form the input of $put \, [\![bx_1]\!]$. The last equality changes two. The first is a change from $dummy$ to an application $construct\_dummy \ v$. This means we need a way to construct $dummy$ from the original view $v$. Programmers can be required to give this $construct\_dummy$ function, but it may be inessential for ill-typed systems where choosing $dummy$ as $v$ is the easiest way to meet our expectation. We set up that choice in our experiments but still use the $construct\_dummy$ function in the later definitions to provide a more general meanning.
%
The second is the change from $(s_3, v_3) \Leftarrow \pg{bx_1}{s}{s_2}$ to $(s_3, v_3') \Leftarrow \pg{bx_1}{s_1}{s_2}$, where $s$ and $v_3$ are replaced with $s_1$ and $v_3'$ respectively. Because $s_1$ is a source update of $\putbx{bx_1}{s}{dummy}$, so under the \textsc{PutPut} law, it is possible to substitute $s$ by $s_1$. The substitution of $v_3$ by $v_3'$ is simply replacing the variable name since $v_3$ and $v_3'$ hold different results of $\getbx{bx_1}{s}$ and $\getbx{bx_1}{s_1}$ respectively. Because both variables are no longer used later, this substitution does not affect the outcome of the function.
...
%-}
    ...
    \noindent $\cpg{bx_1 \circ bx_2}{ks}{kv}{s}{v} =\\
        \tab (ks_1, kv_1, \underline{s_1}, v_1) \Leftarrow \cpg{bx_1}{ks}{id}{s}{construct\_dummy \ v};\\
...
\smallvspace
    $\tab s \Leftarrow s_0; v \Leftarrow v_0; (ks, kv, lks, lkv, s, v) \Leftarrow \kpg{bx}{\lambda \_.s}{id}{id}{id}{s}{v};\\
        \tab \qtab (lks \ s; lkv \ v)$
\smallvspace
        
The beginning of accumulative functions $lks$ and $lkv$ are set as identity function, while $ks$ and $kv$ are initiated as the same with the corresponding ones in $cpg$.
...
%-{
    ...
    \textcolor{red}{
        ...
        \tab s \Leftarrow lks \ s; \quad v \Leftarrow lkv \ v; \quad lks \Leftarrow id; \quad lkv \Leftarrow id;\\
        \tab \textnormal{if} \ h \ s = v \ \textnormal{then} \ (ks, kv, lks, lkv, s, v) \ \textnormal{else } \text{undefined}$
    }
    ...
    \noindent 
    \textcolor{red}{
        $\kpg{\product{bx_1}{bx_2}}{ks}{kv}{lks}{lkv}{s}{v} =\\
        \tab s \Leftarrow lks \ s; \quad v \Leftarrow lkv \ v; \quad lks \Leftarrow id; \quad lkv \Leftarrow id;\\
        \tab (ks_1, kv_1, lks_1, lkv_1, s_1, v_1) \Leftarrow \\
            \tabs{5} \kpg{bx_1}{fst \circ ks}{fst \circ kv}{fst \circ lks}{fst \circ lkv}{s}{v};\\
        \tab (ks_2, kv_2, lks_2, lkv_2, s_2, v_2) \Leftarrow \\ 
            \tabs{5} \kpg{bx_2}{snd \circ ks}{snd \circ kv}{snd \circ lks}{snd \circ lkv}{s}{v};\\
    ...
    \noindent 
    \textcolor{red}{
        $\kpg{RearrS \ f_1 \ f_2 \ bx}{ks}{kv}{lks}{lkv}{s}{v} =\\
    ...
    \noindent 
    \textcolor{red}{
        $\kpg{RearrV \ g_1 \ g_2 \ bx}{ks}{kv}{lks}{lkv}{s}{v} =\\
    ...
    \noindent 
    \textcolor{red}{
        $\kpg{Case \ cond_{sv} \ cond_{s} \ bx_1 \ bx_2}{ks}{kv}{lks}{lkv}{s}{v} =\\
        \tab s \Leftarrow lks \ s; \quad v \Leftarrow lkv \ v; \quad lks \Leftarrow id; \quad lkv \Leftarrow id;\\
        \tab \textnormal{if} \ cond_{sv} \ s \ v \ \&\& \ cond_{s} \ s\\
        \tab \textnormal{then} \ (ks, kv, lks, lkv, s', v') \Leftarrow \kpg{bx_1}{ks}{kv}{lks}{lkv}{s}{v}\\
        \tab \textnormal{else} \ (ks, kv, lks, lkv, s', v') \Leftarrow \kpg{bx_2}{ks}{kv}{lks}{lkv}{s}{v}\\
        \tab \underline{\textnormal{fi}} \ cond_s \ (lks \ s') \ \&\& \ cond_{sv} \ s \ (lkv \ v'); \ \textnormal{return} \ (ks, kv, lks, lkv, s', v')$
    }

    \noindent 
    \textcolor{red}{
        $\kpg{bx_1 \circ bx_2}{ks}{kv}{lks}{lkv}{s}{v} =\\
        \tab (ks_1, kv_1, \underline{lks_1}, lkv_1, \underline{s_1}, v_1) \Leftarrow \kpg{bx_1}{ks}{id}{lks}{id}{s}{construct\_dummy \ v};\\
...
%-}
    ...
    \noindent $\kpg{Skip \ h}{ks}{kv}{lks}{lkv}{s}{v} =\\
        \tab es \Leftarrow lks \ s; \quad ev \Leftarrow lkv \ v; \quad elks \Leftarrow id; \quad elkv \Leftarrow id;\\
        \tab \textnormal{if} \ h \ es = ev \ \textnormal{then} \ (ks, kv, elks, elkv, es, ev) \ \textnormal{else } \text{undefined}$
    ...
    \noindent $\kpg{\product{bx_1}{bx_2}}{ks}{kv}{lks}{lkv}{s}{v} =\\
        \tab es \Leftarrow lks \ s; \quad ev \Leftarrow lkv \ v; \quad elks \Leftarrow id; \quad elkv \Leftarrow id;\\
        \tab (ks_1, kv_1, lks_1, lkv_1, s_1, v_1) \Leftarrow \\
            \tabs{5} \kpg{bx_1}{fst \circ ks}{fst \circ kv}{fst \circ elks}{fst \circ elkv}{es}{ev};\\
        \tab (ks_2, kv_2, lks_2, lkv_2, s_2, v_2) \Leftarrow \\ 
            \tabs{5} \kpg{bx_2}{snd \circ ks}{snd \circ kv}{snd \circ elks}{snd \circ elkv}{es}{ev};\\
        \qtab ( \ con \ ks_1 \ ks_2, \ con \ kv_1 \ kv_2, \\
        \qtab \tab con \ (lks_1 \circ fst) \ (lks_2 \circ snd), \ con \ (lkv_1 \circ fst) \ (lkv_2 \circ snd), \\
        \qtab \tab (s_1, s_2), \ (v_1,v_2) \ )$
    ...
    \noindent $\kpg{Case \ cond_{sv} \ cond_{s} \ bx_1 \ bx_2}{ks}{kv}{lks}{lkv}{s}{v} =\\
        ...
        \tab \textnormal{then} \ (ks', kv', lks', lkv', s', v') \Leftarrow \kpg{bx_1}{ks}{kv}{elks}{elkv}{es}{ev}\\
        \tab \textnormal{else} \ (ks', kv', lks', lkv', s', v') \Leftarrow \kpg{bx_2}{ks}{kv}{elks}{elkv}{es}{ev}\\
        \tab \underline{\textnormal{fi}} \ cond_s \ (lks' \ s') \ \&\& \ cond_{sv} \ \textbf{es} \ (lkv' \ v'); \ \textnormal{return} \ (ks', kv', lks', lkv', s', v')$
...
$\xpg{bx}{s}{v}$
    ...
    \tab \vert \ bx_1 \circ bx_2 \to \\
        \tabs{2} (ks_1,kv_1, lks_1, lkv_1, s_1, v_1) \Leftarrow \kpg{bx_1}{\lambda \_.s}{id}{id}{id}{s}{construct\_dummy \ v};\\
