...
Since there is no real view, this $dummy$ is necessary to pair with original source $s$ to form the input of $put \, [\![bx_1]\!]$. The last equality changes two. The first is a change from $dummy$ to an application $construct\_dummy \ s$. This means we need a way to construct $dummy$ from the current source, $s$. Programmers can be required to give this $construct\_dummy$ function, but it may be inessential for ill-typed systems where choosing $dummy$ as $v$ is the easiest way to meet our expectation. We set up that choice in our experiments but still use the $construct\_dummy$ function in the later definitions to provide a more general meanning.
%
The second is the change from $(s_3, v_3) \Leftarrow \pg{bx_1}{s}{s_2}$ to $(s_3, v_3') \Leftarrow \pg{bx_1}{s_1}{s_2}$, where $s$ and $v_3$ are replaced with $s_1$ and $v_3'$ respectively. Because $s_1$ is a source update of $\putbx{bx_1}{s}{dummy}$, so under the \textsc{PutPut} law, it is possible to substitute $s$ by $s_1$ is possible. The substitution of $v_3$ by $v_3'$ is simply replacing the variable name since $v_3$ and $v_3'$ hold different results of $\getbx{bx_1}{s}$ and $\getbx{bx_1}{s_1}$ respectively. Because both variables are no longer used later, this substitution does not affect the outcome of the function.
