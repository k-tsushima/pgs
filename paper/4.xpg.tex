...
% If the BX programs have efficient inverse way for evaluation, they are not affected by this restriction. Of course, all BX programs do not always have such efficient way. An example restricted by this is evaluating $bsnoc$ with different length of a source and a view.}}. 
This does not happen for the others which is used in the construction of $\pginline{Case}$.

>>>>>>> eb4091502c9ab4c6eae198cca56e5f15745add82
...
The beginning of accumulative functions $lks$ and $lkv$ are set as identity function, while $ks$ and $kv$ are initiated as the same with the corresponding ones in $cpg$. The relationship among $kpg$, $cpg$ and $pg$ can be shown as follows:
