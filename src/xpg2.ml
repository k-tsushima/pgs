...
(*
	s = s0, v = v0
  xpg2 bx ks kv ks' kv' s' v' env = (ks, kv, ks', kv',s', v')
  where
    ks, kv ~ cpg 
    ks', kv', s', v' ~ kpg
*)

let rec xpg2 (bx:bigul) (ks:data->data) (kv:data->data) s' v' env = match bx with
  | Def(name, bx1, bx2) ->
    	xpg2 bx2 ks kv s' v' ((name, bx1) :: env)
  | Var(name) -> ( 
			try
				let bx = snd (List.find (fun (x, t) -> x = name) env) in
					xpg2 bx ks kv s' v' env
			with Not_found -> 
  ...
  | Case(condsv, conds, bx1, bx2) ->
    if (condsv s' v') then
        xpg2 bx1 ks kv s' v' env
    else
        xpg2 bx2 ks kv s' v' env
  | Compose(bx1, bx2) ->
    let (ks1, kv1, s1', v1') =
      xpg2
        bx1
