...
let cpg_skip1 =
  ...
  let v = Unit in
  let (ks, kv, s', v') = cpg skip1 (fun _ -> s) (fun _ -> v) s v [] in
  (s', v')
...
let cpg_replace = 
  ...
  let (ks, kv, s', v') = cpg Replace (fun _ -> s) (fun _ -> v) s v [] in
  (s', v')
...
let cpg_rearrS =
  ...
  let (ks, kv, s', v') = cpg rearrS_d (fun _ -> s) (fun _ -> v) s v [] in
  (s', v')
...
let cpg_rearrV =
  ...
  let v = Int 100 in
  let (ks, kv, s', v') = cpg rearrV_d (fun _ -> s) (fun _ -> v) s v [] in
  (s', v')
...
let cpg_prod =
  ...
  let (ks, kv, s', v') = cpg prod_rs (fun _ -> s) (fun _ -> v) s v [] in
  (s', v')
...
let cpg_phead =
  ...
  let v = Int 100 in
  let (ks, kv, s', v') = cpg phead (fun _ -> s) (fun _ -> v) s v [] in
  (s', v')
...
let cpg_phead_with_case =
  let s = Con(Int 1, Con(Int 2, Unit)) in
  let v = Int 100 in
  let (ks, kv, s', v') = cpg phead_with_case (fun _ -> s) (fun _ -> v) s v [] in
  (s', v')
...
let cpg_phead2 =
  let s = Con(Int 1, Con(Int 2, Unit)) in
  let v = Con(Int 100, Unit) in
  let (ks, kv, s', v') = cpg phead2 (fun _ -> s) (fun _ -> v) s v [] in
  (s', v')
...
let cpg_phead2_with_case =
  let s = Con(Int 1, Con(Int 2, Unit)) in
  let v = Con(Int 100, Unit) in
  let (ks, kv, s', v') = cpg phead2_with_case (fun _ -> s) (fun _ -> v) s v [] in
  (s', v')
...
let cpg_replaceall =
  ...
  let v = Int 100 in
  let (ks, kv, s', v') = cpg replaceAlldef (fun _ -> s) (fun _ -> v) s v [] in
  (s', v')
...
let cpg_bsnoc =
  ...
  let (ks, kv, s', v') = cpg bsnoc_def (fun _ -> s) (fun _ -> v) s v [] in
  (s', v')
...
let cpg_lassoc_comp_phead_with_case n =
  let s = make_smallest_nested_list (n + 1) in
  let v = Int 100 in
  let (ks, kv, s', v') = cpg (lassoc_comp phead_with_case n) (fun _ -> s) (fun _ -> v) s v [] in
  (s', v')

let cpg_rassoc_comp_phead_with_case n =
  let s = make_smallest_nested_list (n + 1) in
  let v = Int 100 in
  let (ks, kv, s', v') = cpg (rassoc_comp phead_with_case n) (fun _ -> s) (fun _ -> v) s v [] in
  (s', v')
...
let cpg_lassoc_comp_phead2 n =
  ...
  let v = Con(Int 100, Unit) in
  let (ks, kv, s', v') = cpg (lassoc_comp phead2 n) (fun _ -> s) (fun _ -> v) s v [] in
  (s', v')

let cpg_rassoc_comp_phead2 n =
  let s = make_consecutive_list n in
  let v = Con(Int 100, Unit) in
  let (ks, kv, s', v') = cpg (rassoc_comp phead2 n) (fun _ -> s) (fun _ -> v) s v [] in
  (s', v')
...
let cpg_lassoc_comp_phead2_with_case n =
  let s = make_consecutive_list n in
  let v = Con(Int 100, Unit) in
  let (ks, kv, s', v') = cpg (lassoc_comp phead2_with_case n) (fun _ -> s) (fun _ -> v) s v [] in
  (s', v')

let cpg_rassoc_comp_phead2_with_case n =
  let s = make_consecutive_list n in
  let v = Con(Int 100, Unit) in
  let (ks, kv, s', v') = cpg (rassoc_comp phead2_with_case n) (fun _ -> s) (fun _ -> v) s v [] in
  (s', v')
