...
let rec get (bx:bigul) s env =
  ...
  | Replace -> s
  | Prod(bx1, bx2) ->
     let v1 = get bx1 ((fun (Con(s1, s2)) -> s1) s) env in
     let v2 = get bx2 ((fun (Con(s1, s2)) -> s2) s) env in
     Con(v1, v2)
  | RearrS(f1, f2, bx) ->
...
| Def(name, bx1, bx2) ->
  ...
  | Replace -> v
  | Prod(bx1, bx2) ->
     let s1 = put bx1 ((fun (Con(s1, s2)) -> s1) s)
                      ((fun (Con(v1, v2)) -> v1) v) env in
     let s2 = put bx2 ((fun (Con(s1, s2)) -> s2) s)
  ...
  | Case(conds, condsv, bx1, bx2) ->
     if condsv s v
     then put bx1 s v env
     else put bx2 s v env
